...
echo "Red Hat JBoss EAP 7.2 Cluster Intallation Start " >> install.progress.txt
...
then
    ...
    subscription-manager attach --pool=${10} >> install.out.txt 2>&1
...
subscription-manager repos --enable=jb-eap-7.2-for-rhel-8-x86_64-rpms >> install.out.txt 2>&1
...
echo "Red Hat JBoss EAP 7.2 Cluster Intallation End " >> install.progress.txt
/bin/date +%H:%M:%S  >> install.progress.txt